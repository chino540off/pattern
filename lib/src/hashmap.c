...
static unsigned long crc32(unsigned char const * s, unsigned int len)
{
...
static unsigned int hashmap_hash_int(hashmap_t const * m, char const * key)
{
    unsigned long index = crc32((unsigned char *)(key), strlen(key));
	...
	index = (index >> 3) * 2654435761;

	return index % m->table_size;
}

static int hashmap_hash(hashmap_t * m, char const * key)
{
	...
	{
		if (m->entries[curr].in_use == 0)
			return curr;

		if (m->entries[curr].in_use == 1 && (strcmp(m->entries[curr].key,key) == 0))
			return curr;
...
static int hashmap_rehash(hashmap_t * m)
{

	/* Setup the new elements */
	hashmap_node_t * temp = calloc(2 * m->table_size, sizeof (hashmap_node_t));
	if (!temp)
		return MAP_OMEM;

	/* Update the array */
	hashmap_node_t * curr = m->entries;
	m->entries = temp;
	...
	for (int i = 0; i < old_size; ++i)
	{
		...
		status = hashmap_put(m, curr[i].key, curr[i].value);
		if (status != MAP_OK)
			return status;
	}
...
hashmap_t * hashmap_new()
{
	...
	m->entries = calloc(INITIAL_SIZE, sizeof (hashmap_node_t));
	if (m->entries == 0)
	{
		free(m);
		return 0;
	...
	for (int i = 0; i < MAX_CHAIN_LENGTH; i++)
	{
        ...
        if (in_use == 1)
		{
            if (strcmp(m->entries[curr].key,key) == 0)
			{
                ...
                m->entries[curr].value = 0;
                m->entries[curr].key = 0;
